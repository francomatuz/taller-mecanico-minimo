...
export class SupabaseService {
    ...
    try {
      ...
      for (const auto of autos || []) {
        ...
        fichas.push({
          ...
          es_service: ultimoServicio?.es_service,
          proximo_service: ultimoServicio?.proximo_service,
  ...
  // Actualizar ficha (actualizar auto + opcionalmente crear nuevo servicio)  
  static async updateFicha(id: number, ficha: any): Promise<{ success: boolean; error?: string; serviceId?: number }> {
    try {
      console.log('🔍 [SUPABASE] Iniciando updateFicha para ID:', id);
      console.log('🔍 [SUPABASE] Datos recibidos:', ficha);
      console.log('🔍 [SUPABASE] Trabajo realizado:', ficha.trabajo_realizado);
      console.log('🔍 [SUPABASE] Kilometraje:', ficha.kilometraje);
      
      // Actualizar datos del auto SIEMPRE
      ...
      if (autoError) {
        console.error('❌ [SUPABASE] Error actualizando auto:', autoError);
        throw autoError;
      }
      ...
      if (tieneActualizacionesServicio) {
        ...
        const { data: ultimoServicio, error: servicioError } = await supabase
          .from('servicios')
          .select('id, es_service, fecha_trabajo')
          .eq('auto_id', id)
          .order('created_at', { ascending: false })
          .limit(1)
          .maybeSingle();
        ...
        if (!ultimoServicio) {
          ...
          if (ficha.es_service !== undefined) {
            ...
            if (ficha.es_service) {
              ...
              if (fechaParaCalcular) {
                ...
                updateData.proximo_service = proximoService.toISOString().split('T')[0];
                console.log('🔧 [SUPABASE] Service marcado, próximo service:', updateData.proximo_service);
                console.log('🔧 [SUPABASE] Fecha base usada:', fechaParaCalcular);
              } else {
                console.log('⚠️ [SUPABASE] No hay fecha_trabajo para calcular próximo service');
                // Si no hay fecha, usar fecha actual + 12 meses
                const proximoService = new Date();
                proximoService.setMonth(proximoService.getMonth() + 12);
                updateData.proximo_service = proximoService.toISOString().split('T')[0];
                console.log('🔧 [SUPABASE] Service marcado con fecha actual + 12 meses:', updateData.proximo_service);
              }
            } else if (!ficha.es_service) {
              // Si se desmarca como service, limpiar próximo service
              updateData.proximo_service = null;
            }
          }
          
          // Actualizar el último servicio
          ...
          if (updateError) {
            ...
            console.log('✅ [SUPABASE] Último servicio actualizado, ID:', ultimoServicio.id);
          }
        }
      }
  ...
  // Función temporal para arreglar servicios existentes que tienen es_service = true pero proximo_service = null
  static async fixExistingServices(): Promise<{ success: boolean; error?: string; fixed: number }> {
    try {
      console.log('🔧 [SUPABASE] Arreglando servicios existentes...');
      
      // Buscar servicios que tienen es_service = true pero proximo_service = null
      const { data: serviciosProblematicos, error: selectError } = await supabase
        .from('servicios')
        .select('id, fecha_trabajo, es_service, proximo_service')
        .eq('es_service', true)
        .is('proximo_service', null);
      
      if (selectError) throw selectError;
      
      console.log(`🔍 [SUPABASE] Encontrados ${serviciosProblematicos?.length || 0} servicios para arreglar`);
      
      if (!serviciosProblematicos || serviciosProblematicos.length === 0) {
        return { success: true, fixed: 0 };
      }
      
      let fixed = 0;
      
      for (const servicio of serviciosProblematicos) {
        if (servicio.fecha_trabajo) {
          // Calcular próximo service basado en fecha_trabajo + 12 meses
          const fechaService = new Date(servicio.fecha_trabajo);
          const proximoService = new Date(fechaService);
          proximoService.setMonth(proximoService.getMonth() + 12);
          
          const { error: updateError } = await supabase
            .from('servicios')
            .update({ proximo_service: proximoService.toISOString().split('T')[0] })
            .eq('id', servicio.id);
          
          if (updateError) {
            console.error(`❌ [SUPABASE] Error arreglando servicio ${servicio.id}:`, updateError);
          } else {
            fixed++;
            console.log(`✅ [SUPABASE] Servicio ${servicio.id} arreglado, próximo service: ${proximoService.toISOString().split('T')[0]}`);
          }
        } else {
          // Si no hay fecha_trabajo, usar fecha actual + 12 meses
          const proximoService = new Date();
          proximoService.setMonth(proximoService.getMonth() + 12);
          ...
          if (updateError) {
            ...
            fixed++;
            console.log(`✅ [SUPABASE] Servicio ${servicio.id} arreglado con fecha actual + 12 meses`);
          }
        }
      }
      
      console.log(`🎉 [SUPABASE] Arreglados ${fixed} servicios`);
      return { success: true, fixed };
      
    } catch (error: any) {
      console.error('❌ [SUPABASE] Error arreglando servicios:', error);
      return { success: false, error: error.message, fixed: 0 };
    }
  }
  ...
  // Obtener todos los servicios de un auto para WhatsApp
  static async getAllServicesForAuto(autoId: number): Promise<any[]> {
    try {
      ...
      console.error(`❌ [SUPABASE] Error obteniendo servicios para auto ${autoId}:`, error);
      return [];
    }
  }
}
